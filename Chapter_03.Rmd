...
What do you conclude? The residuals are not white noise.  The p-value is significant.
...
print(c("Good value of lambda for usnetelec: ", lambda))
autoplot(BoxCox(usnetelec,lambda))
```

* usgdp

```{r usgdp, echo=FALSE, fig.height=4.6}
autoplot(usgdp) +
  xlab("Year") + ylab("") +
...
print(c("Good value of lambda for usgdp: ", lambda))
...
print(c("Good value of lambda for mcopper: ", lambda))
...
print(c("Good value of lambda for enplanements: ", lambda))
...
Box-Cox transformation does not yield a better model.
...
Bias adjusted Box-Cox Transformation with lambda = `r lambda`.
...
The ACF plot result indicates that the residuals are not white noise because of significant spike at lag 4. The Ljung-Box test shows that it is statistically significant. The residuals are not white noise and are not normally distributed.
...
The ACF plot indicates that the residuals are not white noise because of existence of significant spikes. The Ljung-Box test shows that they are statistically significant. The residuals are not white noise and are not normally distributed.

The naive method is prefered because there are not any particular seasonal pattern in the data and the Q values of Ljung-Box test were same for both methods.
...
The ACF plot indicates that the residuals are notwhite noise because of existence of significant spikes. The Ljung-Box test shows that they are statistically significant. The residuals are not white noise and are not normally distributed.
...
Ans: It is not necessary for good forecast methods to have normally distributed residuals
- A model with small residuals will give good forecasts.
Ans: Model does not need to have small residuals but their residuals should be uncorrelated and have zero mean.
- The best measure of forecast accuracy is MAPE.
Ans: Best measure of forecast accuracy is different on a case by case basis.
- If your model doesn’t forecast well, you should make it more complicated.
Ans: More complicated forecast method does not guarantee better forecast.
- Always choose the model with the best forecast accuracy as measured on the test set.
Ans:  Choose a model with residuals with zero mean and uncorrelated with each other. Choosing best accuracy model is not always the best choice.
...
```{r}
accuracy(snaive_ibm, ibmclose_test)
accuracy(mean_ibm, ibmclose_test)
accuracy(naive_ibm, ibmclose_test)
accuracy(drift_ibm, ibmclose_test)
```
...
```{r}
e_snaive_ibm <- ibmclose_test - snaive_ibm$mean
e_naive_ibm <- ibmclose_test - naive_ibm$mean
e_drift_ibm <- ibmclose_test - drift_ibm$mean
e_mean_ibm <- ibmclose_test - mean_ibm$mean
...
The residuals does not resemble white noise.